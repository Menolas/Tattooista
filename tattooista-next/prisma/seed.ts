...
async function main() {
  ...
  const contactsPage = await prisma.page.upsert({
    where: { name: "contacts" },
    update: {},
    create: {
      name: "contacts",
  ...
  console.log("Created pages:", { aboutPage: aboutPage.name, contactsPage: contactsPage.name })

  // Create some tattoo styles
  const styles = [
    {
      value: "Traditional",
      description:
